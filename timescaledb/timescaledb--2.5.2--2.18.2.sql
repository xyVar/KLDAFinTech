...
BEGIN
  SELECT value INTO catalog_version FROM _timescaledb_catalog.metadata WHERE key='timescaledb_version' AND value <> '2.5.2';
  IF FOUND THEN
    RAISE EXCEPTION 'catalog version mismatch, expected "%" seen "%"', '2.5.2', catalog_version;
  END IF;
END$$;
...
CREATE FUNCTION @extschema@.get_telemetry_report() RETURNS jsonb
    AS '$libdir/timescaledb-2.18.2', 'ts_telemetry_get_report_jsonb'
...
AS '$libdir/timescaledb-2.18.2', 'ts_recompress_chunk_segmentwise' LANGUAGE C STRICT VOLATILE;
CREATE FUNCTION _timescaledb_internal.get_compressed_chunk_index_for_recompression(REGCLASS) RETURNS REGCLASS
AS '$libdir/timescaledb-2.18.2', 'ts_get_compressed_chunk_index_for_recompression' LANGUAGE C STRICT VOLATILE;
